...
### read_batch_info
This is technical/Batch related information ie RIN, RNA concentrations, proceesing dates etc etc
...
Many studies we see like to run the same samples more than once...this is a check for any duplicate sample ids the data provider/owners arent aware of.
...
Here we merge all the sample related information
...
# PCA plots
# plot to screen
pca_plot_lumi(eset_raw, probe_list=rownames(eset_raw) )
par(def.par)
```
...
```{r writing_final_good_probe_list}
##################################
...
## Get list of HOUSEKEEPING probes and  probes_not_detected_in_any_sample
```{r housekeeping_and_notDetected_probes, eval=TRUE}
my_eset <- eset_bg
# id probes not expressed
## probe annotations
probes_not_detected_in_any_sample <- probe_detection==0
##  Housekeeping probes
x <- getControlData(my_eset)
x <- subset(x, x$controlType=="HOUSEKEEPING")
house_probes <- x$ProbeID
house_probes_nuID <- fData(my_eset)$nuID[fData(my_eset)$ProbeID %in% house_probes]
## fData(my_eset)$TargetID[fData(my_eset)$ProbeID %in% house_probes]
## [1] "ACTB"   "EEF1A1" "GAPDH"  "RPS9"   "TUBB2A" "TXN"    "UBC
gx <- t(exprs(my_eset))
gx_new <- gx[,house_probes_nuID]
probe_list <- c(house_probes_nuID, colnames(gx)[probes_not_detected_in_any_sample])
housekeeping_and_notDetected_probes <- probe_list
save(housekeeping_and_notDetected_probes, file=paste(out_dir,"/",project_name,".housekeeping_and_notDetected_probes.RData",sep=""))
```
...
source("https://github.com/snewhouse/BRC_MH_Bioinformatics/sjnewhouse_misc_R_SampleNetwork_v2.R")

# basic_sampleNetworkIterate

ISAoutliers <- basic_sampleNetworkIterate(gene_expression_matrix=exprs(eset_bg_log2_rsn_0),
                                          phenotype_data=pData(eset_bg_log2_rsn_0),
                                          groupsLabel="GROUPS", 
...
# PCA plots
# plot to screen
pca_plot_lumi(eset_bg_log2_rsn, probe_list=rownames(eset_bg_log2_rsn))
par(def.par)
...
StepAIC is used to id best set of tech/batch variables that associate with PC1
...
batch_var_names
## Run PCA
cat(" Running PCA on t(exprs(eset)) using housekeeping probes and probes not detected in any sample","\r")
gx_new <- gx[,housekeeping_and_notDetected_probes]
#' new pca based on Housekeeping genes and probes not expressed in any sample
pca_gx_pcs <- prcomp(gx_new,scale=TRUE)$x
pca_gx_pc1 <- pca_gx[,"PC1"]
head(pca_gx_pc1)
save(pca_gx_pcs,pca_gx_pc1, file=paste(out_dir,"/",project_name,".eset_bg_log2_rsn_c_pca_gx_pcs.RData",sep=""))
```

## 2 make_phenotype_for_regressions
```{r make_phenotype_for_regressions, eval=TRUE}
# PHENOTYPES FOR REGRESSIONS
cat(" setting up phenotypes PC1,PHENOTYPE & GROUPS for regressions ","\r")
PC1 <- as.numeric(pca_gx_pc1)
PHENOTYPE <- factor(toupper(pData(eset_lumiN)$PHENOTYPE),levels=c("CO","CO_AD","AD")) # toupper() called because of pesky 'case' issues
GROUPS <-    factor(toupper(pData(eset_lumiN)$GROUPS)) # toupper() called because of pesky 'case' issues
TISSUE <- factor(toupper(pData(eset_lumiN)$TISSUE))
#
techdf <- cbind(tech_batch,PHENOTYPE,GROUPS,TISSUE,PC1)
head(techdf)
tech_batch <- techdf
head(tech_batch)
tech_batch$tech.Sentrix.Barcode <- as.factor(tech_batch$tech.Sentrix.Barcode)
tech_batch$tech.Batch <- as.factor(tech_batch$tech.Batch)
tech_batch$Signal.Average <- as.numeric(tech_batch$Signal.Average)
str(tech_batch)

#' correlate numerical var
head(tech_batch)
num_class <- sapply(tech_batch,class)=="numeric"
num_var <- tech_batch[,num_class]
fac_var <- tech_batch[,sapply(tech_batch,class)=="factor"]
date_var <- tech_batch[,grep("Date",names(tech_batch))]
head(date_var)

cor_var <- round( cor(num_var), 2)
cor_var
heatmap.2(cor_var,col="greenred")

#' from caret
highlyCorVar <- findCorrelation(cor_var, cutoff = 0.75, verbose=FALSE)
filteredVar <- num_var[, -highlyCorVar]

cor_var2 <- cor(filteredDescr)
head(filteredVar)
summary(cor_var2[upper.tri(cor_var2)])
heatmap.2(cor_var2,col="greenred")

# count factors
sapply(fac_var,table)
sapply(date_var,table)

#' remove tech.Date_RNA_Extracted
date_var <- date_var[,-(grep("tech.Date_RNA_Extracted",names(date_var)))]
head(date_var)

#' new tech_data
tech_batch_new <- cbind(filteredVar,date_var,fac_var,PC1) 
head(tech_batch_new)
names(tech_batch_new)

batch_var_names <- names(tech_batch_new)[3:14]
batch_var_names <- sample(batch_var_names, size=length(batch_var_names), replace=FALSE)
batch_var_names
```
...
if(min_anova_p <= 0.05) {
  ...
  # best model
  best_model <- paste(anova_data$terms[anova_data$"Pr(>F)" < 0.05/dim(anova_data)[1]], collapse=" + ")
...
cat(" BEST MODEL BASED ON stepAIC [",pheno,"] ~ [",paste(best_model)[3],"] ","\r","\n")
  ...
  ## ANOVA TABLE FOR SIG TERMS ONLY
  anova_data$Iteration <- i
  anova_data <- subset(anova_data, anova_data$"Pr(>F)" < 0.05/dim(anova_data)[1])
  anova_data
  
  ## GET AND MAKE MODEL FORMULA
  best_terms <- rbind(paste(best_model)[3],best_terms)
  anova_all_models <- rbind(anova_data,anova_all_models)
  paste(best_model)[3]

}


# counts of N times a variable is selected 
count_terms <- table(anova_all_models$terms)
count_terms <- as.data.frame(count_terms)
count_terms <- count_terms[order(count_terms$Freq,decreasing=TRUE),]
count_terms

model_terms <- paste(count_terms$Var1,collapse="+")
model_terms
```

Run regression 100 times, shuffling the order of the predictors each time. The following counts are the number of times a term gets selected from the `stepAIC` and is significant at `p<0.05/N_TERMS` from the `anova`.
Use N rounds of ComBat to correct for N catergorical/date variables id'd from `stepAIC` .  
After ComBat use linear regression to adjust for any numerical variables from `stepAIC` 

ComBat
==============================
Example below is for 3 sig batch variables.
This is a quick example, I recommend running PCA batch regressions after each round of ComBat

```{r Combat_all_probes}
library(sva)

my_eset <- eset_bg_log2_rsn
...
# Batch Var
batch0 <- as.factor(pheno$tech.Sentrix.Barcode)
batch1 <- as.factor(pheno$tech.Date_Tissue_Collected)
batch2 <- as.factor(pheno$tech.Date_cDNA_Amplification)

# model matrix
mod_plus_covar <- model.matrix(~ as.factor(GROUPS) + tech.Amplication_A280 + tech.bioanalyser_RNA_RIN , data=tech_batch_new)
colnames(mod_plus_covar)

## Correct for batch using ComBat
# Batch = Sentrix.Barcode
combat_edata0 <- ComBat(dat=edata, batch=batch0, mod=mod_plus_covar, numCovs=c(13,14),par.prior=TRUE, prior.plots=TRUE)
# Batch = Date_Tissue_Collected
combat_edata1 <- ComBat(dat=combat_edata0, batch=batch1, mod=mod_plus_covar, numCovs=c(13,14),par.prior=TRUE, prior.plots=TRUE)
# Batch = Date_cDNA_Amplification
combat_edata2 <- ComBat(dat=combat_edata1, batch=batch2, mod=mod_plus_covar, numCovs=c(13,14),par.prior=TRUE, prior.plots=TRUE)


# replace exprs
eset_bg_log2_rsn_combat <- my_eset
exprs(eset_bg_log2_rsn_combat) <- combat_edata2
pData(eset_bg_log2_rsn_combat)$PC1_all_pre_ComBat <- PC1

# save 
save(eset_bg_log2_rsn_combat, file=paste(out_dir,"/",project_name,".eset_bg_log2_rsn_combat.RData",sep="")  , compress=T)
```


Batch Correction using linear models
=====================================
ComBat does not remove effects of linear/qunatitative variables. Do this with linear regresssion.
The other option is to use `sva` or `prcomp` and use the SVs or PCs in analyses and/or regress them out at this stage
Again, run PCA regressions from a bove and check you have actually removed that varitation and lool at qc plots - coloured dendograms!

Uses `RcppArmadillo` for ***FAST REGRESSSION****
...
gx <- exprs(eset_bg_log2_rsn_combat)
n_probes <- dim(gx)[1]
gx <- t(gx)
dim(gx)
gx[1:10,1:10]
...
# get batch phenos
batch_pheno <- tech_batch[,paste(count_terms$Var1)]
head(batch_pheno)
batch_pheno <- cbind(batch_pheno[,c(1,5)],gx)
# this is the data for the regression
batch_pheno[1:10,1:10]
#
eset_bg_log2_rsn_combat_regression_input <- batch_pheno

save(eset_bg_log2_rsn_combat_regression_input,file=paste(out_dir,"/",project_name,".eset_bg_log2_rsn_combat_regression_input.regression_input.RData",sep=""))
...
#summary(flmmod)$adj.r.squared
best_model <- paste(count_terms$Var1[c(1,5)],collapse="+")
best_model
...
for(probe in new_probe_names){
    ...
    lm_probe <-  fastLm( lm_model , data=eset_bg_log2_rsn_combat_regression_input)
    ...
    sink(file=paste(out_dir,"/",project_name,".eset_bg_log2_rsn_combat_regression_input.lm_probe_progress_rsq.txt",sep=""),append=TRUE)
    cat(" doing [",probe ,"] ~ [",paste(best_model),"].RSQ=",rsq,". Progress:",round(pn/n_probes,3),"\r","\n")
    sink()
    pn <- pn + 1
    }
...
adj_gx <- t(adj_gx)
rownames(adj_gx) <- rownames(exprs(eset_bg_log2_rsn_combat))
adj_gx[1:10,1:10]

# save raw matrix
eset_bg_log2_rsn_combat_adj_gx <- adj_gx
save(eset_bg_log2_rsn_combat_adj_gx,file=paste(out_dir,"/",project_name,".eset_bg_log2_rsn_combat_adj_gx.RData",sep=""))
```
...
eset_bg_log2_rsn_combat_adj <- eset_bg_log2_rsn_combat;
exprs(eset_bg_log2_rsn_combat_adj) <- adj_gx;
min(exprs(eset_bg_log2_rsn_combat_adj))
# save eset_bg_log2_rsn_adj
save(eset_bg_log2_rsn_combat_adj, file=paste(out_dir,"/",project_name,".eset_bg_log2_rsn_combat_adj.RData",sep="")  , compress=T)
```

******

QC Plots of `eset_bg_log2_rsn_combat_adj`
---------------------------------------------------------------
## basic_qc_plot_lumi 
